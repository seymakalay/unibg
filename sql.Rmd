...
Order of the comments are important. You can memorize them easily by locating the first letter in your keybord.
...
and add all the functions should be aggregated (`SUM`,`AVG`,`MIN`,etc).
...
let\'s apply filter to GROUP BY using `HAVING` statement.
```
SELECT
